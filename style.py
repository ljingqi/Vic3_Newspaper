...
DOP_NOTES = {
    ...
    "law_autocracy": "当前为专制政体，报纸须拥护君主与现行体制，不得妄议国政。",
...
MODERNITY_TIERS = {
    1: {
        ...
        "voice": (
            ...
            "铁律：只能基于给定事实合理演绎，不编造具体数字或国家名；"
            "数据缺失时相应内容简写或略去，不得编造；行文中以「本馆」指代本报刊名。"
        ),
        "econ_guide": (
            "经济板块首句必须以「据户部奏报，国用出入以……计」（填入给定GDP数值）"
            "引出经济总量，如「据户部奏报，国用出入以四千六百余万两计」；"
    ...
    2: {
        ...
        "econ_guide": (
            "经济板块首句必须以「据户部消息，我国国民生产总值为……」（填入给定GDP数值）"
            "引出经济总量，如「据户部消息，我国国民生产总值为430,521,263英镑」；"
            "人口、生活水平、识字率等其余指标同样以旧式公文笔法展开。"
        ),
        "ads_guide": (
            "广告栏须为20世纪初报刊告白体：商品告白、工艺铺面招贴、学堂晓谕、书画社启事皆可，"
    ...
    3: {
        ...
        "voice": (
            ...
            "等语汇，消息客观、评论持重，措辞审慎而不空喊口号；有喜报喜、有忧报忧，"
            "以建设与发展为主线，不轻佻浮夸。{ERA}{VOTE}使用简体中文与 Markdown。"
            "铁律：只能基于给定事实合理演绎，不编造具体数字或国家名；"
            "数据缺失时相应内容简写或略去，不得编造；行文中以「本报」指代本报刊名。"
        ),
        "econ_guide": (
            "经济板块首句必须以「国家统计局最新数据显示，我国GDP为……」（填入给定GDP数值）"
            "引出经济总量，如「国家统计局最新数据显示，我国GDP为430,521,263英镑」；"
            "人口、生活水平、识字率等其余指标以官方书面语展开。"
        ),
        "ads_guide": (
            "广告栏须为现代报纸广告/启事体：国营厂矿与工商产品广告、展览会通知、"
            "招生启事、征订启事等，措辞正式简明，突出产品与服务，不得使用旧式文言告白腔。"
        ),
        "number_format": "arabic",
        "number_guide": (
            "一律使用阿拉伯数字并加千分位分隔符（如46,077,267英镑、21,862,816人、69.45%），"
            "不得使用汉字数字。"
        ),
        "section_titles": {
            "headline": "今日要闻",
    ...
    4: {
        ...
        "voice": (
            ...
            "标题朴实有力，不堆砌形容词；社论与调查报道发达，重视数据、逻辑与公共政策讨论，"
            "注重权威与可信，不渲染、不夸张。{ERA}{VOTE}使用简体中文与 Markdown。"
            "铁律：只能基于给定事实合理演绎，不编造具体数字或国家名；"
            "数据缺失时相应内容简写或略去，不得编造；行文中以「本社」指代本通讯社。"
        ),
        "econ_guide": (
    ...
    5: {
        ...
        "voice": (
            ...
            "标题新颖而不失庄重。{ERA}{VOTE}使用简体中文与 Markdown。"
            "铁律：只能基于给定事实合理演绎，不编造具体数字或国家名；"
            "数据缺失时相应内容简写或略去，不得编造；行文中以「本刊」指代本刊名。"
        ),
        "econ_guide": (
