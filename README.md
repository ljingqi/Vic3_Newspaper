...
3. **创建配置文件**：仓库已提供 `config.example.json`（所有参数已填好，仅 API Key 留空），
   ...
   - **功能开关**：`newspaper_enabled` / `magazine_enabled` 控制自动管线中的报纸/杂志生成；
     `epidemic_enabled` 控制疫情专题（报纸「疫情专电」板块 + 杂志第4篇疫情特稿，见下文）；
     ...
     `report_date_random_enabled`（报告日期随机化，默认开：自动存档恒在 1.1，报告月恒为 1
     会让时令风味永远停在冬季；开启后每存档年抽一个出版日期——年份=存档年、月份∈[存档月,12]，
     写入报头/提示词日期并驱动时令取材，真实存档日期另存 `save_date` 供法律窗口与战争判定用），
     `report_date_salt` 填任意串可重抽该日期（留空 = 每存档年真随机一次）；
...
- **法律**：**本年度新施行/废除的法律**明细（逐年对比可侦测法律变化）；另附现行**言论自由**法律状态以增风味。**法律附则（游戏内 amendment）**按新旧分流：本报告年新增的附则进政界动态素材，开局即有的既有附则只列在程序端《现行宪法要览》附则段（避免把多年未变的附则年年写成本年新立法）
...
档位之外还有一条**文化圈（sphere）轴**，解决「西方专制国被写成中式邸报、并用农历纪年」的问题。
文化圈由存档 `player_tag` 主表（`data/country_sphere.json`）判定，国教与文化名兜底：

| 文化圈 | 覆盖 | 1~2 档语域 |
| --- | --- | --- |
| `sinic` | 汉/满/蒙古/藏/朝鲜/日本/越南… | **沿用原五档模板**（邸报体，逐字节不变） |
| `west` | 欧洲/美洲/大洋洲 | 1 档「旧制度官报体」（陛下/宫廷/内阁/敕令/公报，公历），2 档「党派大报体」（社论/通讯/据悉，公历） |
| `islam` | 阿拉伯/波斯/突厥/萨赫勒穆斯林政权 | 1 档「王廷公报体」（苏丹/沙阿/汗/埃米尔/迪万，公历），2 档「改良立宪报刊」 |
| `other` | 南亚/东南亚/非洲/美洲原住民/太平洋 | 1 档「王廷或部族公报体」，2 档「近代报刊」 |

- 非中华文化圈统一**公历纪年**、**阿拉伯数字**，并同步替换杂志基调、拟题指南、
  州情速写（衙门→官署、士绅→乡绅、州县→地方）与杂志板块名（《法网与衙门》→《法网与官署》）；
- 3 档非中华文化圈只换机构称谓（「国家统计局」→「官方统计」）与纪年规则，4~5 档不动；
- 文化圈表由 `python tools/gen_country_sphere.py` 从游戏 `common/cultures` 的 `heritage`
  生成（升级游戏后重建）；离线自检：`python tools/test_style_sphere.py`；
  `python tools/run_test4.py --dry` 的清单会显示每国 `sphere`。
...
仅在当年有活跃疫情时向报纸插入「疫情专电」板块、向杂志加入第4篇疫情特稿《疫海浮生》：

- **触发**：每州每年基础概率（约1.5%），受以下因素修正——
  游戏收成条件 **疫病爆发**（`harvest_condition_manager` 中该州域的 `disease_outbreak`）概率×4；
  城市化越高（城邑建筑就业占比）、污染越重越易发；卫生法律、卫生机构投资、相关科技
  （现代排水/现代护理/奎宁）、识字率可降低概率与烈度（缓解系数最高50%）。
- **疾病库**：19世纪至20世纪初15种传染病（霍乱/天花/鼠疫/伤寒/斑疹伤寒/肺结核/麻疹/
  1918流感/黄热病/疟疾/痢疾/白喉/百日咳/猩红热/脊髓灰质炎），R0/病死率为现代回顾性估计
  （见 `疫情专题_流行病学数据_1836-1936.md`，全部带来源），仅用于程序内推演，提示词只给
  模型最终数字（染病/死亡/感染率）。
- **扩散**：按州域相邻表（`state_adjacency.json`，由 `tools/build_state_adjacency.py`
  从游戏地图一次性生成）向相邻州掷骰扩散，亦可越境传入邻国（在外交风云板块提及）。
- **时长**：普通疫情1~3年，特别严重（烈度≥4）可至5年；**1918–1919 西班牙流感为强制
  高概率高烈度事件**（三波袭来）。
