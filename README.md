...
| 命令 | 作用 |
| --- | --- |
| `python journal.py watch [--force]` | 持续监控 debug.log，新数据到达即生成（旧 debug_log 路线，仅保留） |
| `python journal.py once [日志路径]` | 扫描一份已有 debug.log（旧 debug_log 路线，仅保留） |
...
| `python journal.py check` | 自检旧 debug_log 路线：日志路径、|JOURNAL| 标记是否出现、API Key 是否配置 |
...
存档直读路线从 `.v3` 存档中提取精确数据（GDP、生活水平、识字率等），部分无法精确
读取的指标（如商品涨跌、民族宗教占比）以档位形式给出，足够 LLM 把握形势：
...
> 存档直读路线可提供**精确数值**（GDP、生活水平、识字率等），见下文「存档直读路线」。
...
> 旧版 debug_log 路线（`journal.py watch/once` + 已废弃的报纸 mod）的备选方案
> （debug 模式、Text 存档格式等）不再适用于当前版本。
